########## MIKE SHE plugin: OL water depth dynamics and turning point analysis ##########
# Subject:      Reads the overland flow results written by MIKE SHE (_overland.dfs2) after a
#               simulation and computes six static 2D grids from the full transient time series
#               of "depth of overland water":
#                 1. Time mean:              average water depth per cell over the full simulation
#                 2. Mean absolute change:   mean of |delta depth| between consecutive time steps,
#                                            indicating how dynamically active each cell is
#                 3. Turning point count:    number of times the water depth reverses direction per
#                                            cell (i.e. switches from rising to falling or vice
#                                            versa), indicating oscillatory / unstable behaviour
#                 4. Max:                    maximum water depth observed in any single time step
#                 5. Min:                    minimum water depth observed in any single time step
...
#               Output is written to the model result folder as "OL_DepthDynamics.dfs2".
...
# Name of the OL water depth item as written by MIKE SHE into _overland.dfs2.
item_name_ol_depth = "depth of overland water"  # (m), 2D item
...
def _count_turning_points(values):
  ...
  # A turning point is where the water depth switches from rising to falling or vice versa.
...
def leaveSimulator():
  ...
  # Construct the path to the OL result file using MIKE SHE's default result folder convention.
  ...
  ds     = mikeio.open(ol_res_file).read(items=[item_name_ol_depth])
  item   = ds[item_name_ol_depth]  # shape: (time, y, x) as read by mikeio
  values = item.values
  ...
  # --- Item 1: Time mean ---
  # Average water depth per cell over the full simulation period.
  mean_values              = np.nanmean(values, axis=0, keepdims=True)       # (1, y, x)
  mean_values[0, nan_mask] = np.nan
  mean_item = mikeio.DataArray(
    mean_values,
    time=[ds.time[0]],
    item=mikeio.ItemInfo(item_name_ol_depth + " (Mean)", itemtype=item.item.type, unit=item.item.unit),
    geometry=item.geometry
  )

  # --- Item 2: Mean absolute change per timestep ---
  # Mean of |delta depth| between consecutive timesteps. High values indicate strongly
  ...
  dma_item = mikeio.DataArray(
    dma_values,
    time=[ds.time[0]],
    item=mikeio.ItemInfo(item_name_ol_depth + " (Mean abs. change per timestep)", itemtype=item.item.type, unit=item.item.unit),
    geometry=item.geometry
  )

  # --- Item 3: Turning point count ---
  # Number of times the water depth changes direction per cell. High values indicate oscillatory
  # behaviour which may point to numerical instability or strong OL forcing variability.
  ...
  count_item = mikeio.DataArray(
    count_values,
    time=[ds.time[0]],
    item=mikeio.ItemInfo(item_name_ol_depth + " (Turning point count)", itemtype=item.item.type, unit=item.item.unit),
    geometry=item.geometry
  )

  # --- Item 4: Max ---
  # Maximum water depth observed in any single time step per cell.
  # Useful for flood extent mapping and identifying peak inundation areas.
  max_values              = np.nanmax(values, axis=0, keepdims=True)         # (1, y, x)
  max_values[0, nan_mask] = np.nan
  max_item = mikeio.DataArray(
    max_values,
    time=[ds.time[0]],
    item=mikeio.ItemInfo(item_name_ol_depth + " (Max)", itemtype=item.item.type, unit=item.item.unit),
    geometry=item.geometry
  )

  # --- Item 5: Min ---
  # Minimum water depth observed in any single time step per cell.
  min_values              = np.nanmin(values, axis=0, keepdims=True)         # (1, y, x)
  min_values[0, nan_mask] = np.nan
  min_item = mikeio.DataArray(
    min_values,
    time=[ds.time[0]],
    item=mikeio.ItemInfo(item_name_ol_depth + " (Min)", itemtype=item.item.type, unit=item.item.unit),
    geometry=item.geometry
  )

  # --- Item 6: Range (Max - Min) ---
  # Difference between the maximum and minimum water depth per cell.
  ...
  range_item = mikeio.DataArray(
    range_values,
    time=[ds.time[0]],
    item=mikeio.ItemInfo(item_name_ol_depth + " (Range)", itemtype=item.item.type, unit=item.item.unit),
    geometry=item.geometry
  )
  ...
  ds_result.to_dfs("OL_DepthDynamics.dfs2")
